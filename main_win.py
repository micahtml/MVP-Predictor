...
# Print DataFrame columns to check column names
print(df.columns)  # Debugging line to confirm structure
...
# Check available columns again before proceeding
print(df.columns)  # Debugging line to verify structure
...
try:
    mvp_df = (
        df[["Player", "Pos", "Age", "Tm", "G", "MP", "PTS", "TRB", "AST", "MVP_Score"]]
        .sort_values("MVP_Score", ascending=False)
        .head(5)
    )
except KeyError as e:
    print("KeyError: ", e)  # This will help pinpoint the missing columns

# If no error, print the top MVP candidates
if "mvp_df" in locals():
